...
class logger():
    ...
    def launchServer(self):
        #init subscriber
        pose_subscriber         = rospy.Subscriber('/hsr_wagon_world', PoseStamped, self.wagonPoseCallback)
        ...
        time_subscriber    = rospy.Subscriber('/hsr_wagon_time', Header, self.timeCallback)
    ...
    def wagonPoseCallback(self,pose):
        if self.isLogging:
            ...
            self.time.append(rospy.get_time()-self.startTime)
    ...
    def timeCallback(self,data):
        self.laserTime = data.stamp
        if self.isLogging:
            self.time.append(self.laserTime.to_sec()-self.startTime)
            print(self.laserTime.to_sec()-self.startTime)
